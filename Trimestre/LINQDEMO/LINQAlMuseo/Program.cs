...
Console.WriteLine("Potrei anche usare la Select: gruppo -> nuovo oggetto");
opere.GroupBy(o => o.FkArtista).Select(group => new { group.Key, NumeroOpere = group.Count() }).Where(g => g.NumeroOpere >= 2)
     .Join(artisti, a2 => a2.Key, a => a.Id, (a2, a) => new { ID = a.Id, NomeArtista = a.Cognome, a2.NumeroOpere }).ToList().ForEach(Console.WriteLine);
...
Console.WriteLine("Con pipeline (meglio non occupo memoria)");
opere.Join(personaggi, o => o.Id, p => p.FkOperaId, (o, p) => o).ToList().ForEach(Console.WriteLine);
Console.WriteLine();
var opereConPersonaggiConDistinct = opere.Join(personaggi, o => o.Id, p => p.FkOperaId, (o, p) => o).Distinct();
foreach (var opera in opereConPersonaggiConDistinct)
    Console.WriteLine(opera);
