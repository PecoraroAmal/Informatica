...
namespace LINQGym
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nSelezioniamo solo quelli che verificano la condizione e hanno indice pari");
            ...
            studentResultList.
                ForEach(s => Console.WriteLine(s.StudentName + " age = " + s.Age));
            Console.WriteLine("Su una sola riga:");
            studentList1.OrderBy(s => s.Age).ToList().ForEach(s => Console.WriteLine(s.StudentName + " age = " + s.Age));
            ...
            Console.WriteLine("\nstampa su list");
            studentResultList.ForEach(s => Console.WriteLine(s.StudentName + " age = " + s.Age));
            //Studiamo la clausola select - proiettiamo gli elementi della sequenza in una nuova forma
            Console.WriteLine("\nClausola select - LINQ method");
            Console.WriteLine("\nUso di tipi anonimi");
            //uso di tipi anonimi
            Array.ForEach(studentArray1.Select(s => new { Nome = s.StudentName, Eta = s.Age }).ToArray(),Console.WriteLine);
            studentList1.Select(s => new { Nome = s.StudentName, Eta = s.Age }).ToList().ForEach(p => Console.WriteLine(p.Nome));
            Console.WriteLine("\nUso di tipi non anonimi");
            studentList1.Select(s => new Persona(){ Nome = s.StudentName, Eta = s.Age }).ToList().ForEach(p => Console.WriteLine(p.Nome));
            //Group By
            Console.WriteLine("\n\n\nGroup By");
