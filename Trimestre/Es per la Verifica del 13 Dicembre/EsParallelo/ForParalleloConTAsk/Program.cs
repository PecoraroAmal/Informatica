    ...
    class MyData
    {
        ...
        internal class Program
        {
            ...
            static MyData SQ(int n)
            {
                ...
                object _lock = new();
                //Calcolo Parallelo
                //0 inizializza subtotal
                Parallel.For<double>(1, N + 1, () => 0, (i, loopState, subtotal) =>
                {
                    //Calcolo ad ogni loop
                    checked
                    {
                        subtotal += 1.0 / ((long)i * i);
                    }
                    return subtotal;
                },
                //Questa lambda prende in input il valore finale Sq e lo usa per il calcolo completo
                (x) =>
                {
                    lock(_lock)
                    {
